<!--
  ...
  Owner command: agents.generate
  Edit instead: the agents.generate generator source (not this file).
  Regenerate:   pnpm exec werkstatt run agents.generate --site forge-warpgogol
-->

# forge-warpgogol Site Agent Guide
...
This site is an RFC-0047 compliant app. Its readiness gate is `sites-check.run --site forge-warpgogol` plus `app.contract.full --site forge-warpgogol`.

**Do not use this app as a copy source for new sites.** New sites are built exclusively via `.agents/workflows/00-prepare.md` → `06-handoff.md`. The correct way to start a new app is `onboarding.scaffold` inside the `02-scaffold` workflow, never `cp -r apps/forge-warpgogol apps/<new-id>`.
...
2. Add a matching `pages[]` entry in `src/content/system.md` with `pageId`, `routes: { en: ..., ... }`, `cosmicStar`, and `planets[]`.
...
```sh
pnpm --filter forge-warpgogol exec astro check
```
...
- Onboarding scaffold may run `pnpm --filter forge-warpgogol run build` for the first build of a new app.
- Deploy workflow may run `pnpm --filter forge-warpgogol run build:deploy:main` or `build:deploy:alt`.
...
## Cache-clone guards (RFC-0821, RFC-0949)

When working in a mission workpiece, agents MUST follow these cache-clone rules:

- Use `mission.git.commit` to commit workpiece changes — never commit directly to the cache clone or use `git commit --no-verify` to bypass the pre-commit hook.
- When `mission.materialize` fails on content validation, fix the source files in the **cache clone**, not in the workpiece.

See `docs/authoring/site-composition.md §Mission lifecycle operations` for the full rules including env persistence (RFC-0822) and mirror sync (RFC-0472, RFC-0574).

## Deployment pipeline reference (RFC-0949)

For the full 10-step promote-to-main deployment pipeline, see `docs/authoring/site-composition.md §Deployment pipeline: promote-to-main order`.
...
This site uses the **Handwerk · Concrete Blueprint biome** (`handwerk-material-warm`). The default language is `en`; supported languages: `en`.
...
| `AGENTS.md` | `pnpm exec werkstatt run agents.generate --site forge-warpgogol` | After shared AGENTS.md patterns change (RFC-0079) |
