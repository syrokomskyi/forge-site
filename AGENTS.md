<!--
  ...
  Regenerate:   pnpm exec werkstatt run agents.generate --site forge-site
-->

# forge-site Site Agent Guide
...
This site is an RFC-0047 compliant app. Its readiness gate is `sites-check.run --site forge-site` plus `app.contract.full --site forge-site`.

**Do not use this app as a copy source for new sites.** New sites are built exclusively via `.agents/workflows/00-prepare.md` → `06-handoff.md`. The correct way to start a new app is `onboarding.scaffold` inside the `02-scaffold` workflow, never `cp -r apps/forge-site apps/<new-id>`.
...
```sh
pnpm --filter forge-site exec astro check
```
...
- Onboarding scaffold may run `pnpm --filter forge-site run build` for the first build of a new app.
- Deploy workflow may run `pnpm --filter forge-site run build:deploy:main` or `build:deploy:alt`.
...
| `AGENTS.md` | `pnpm exec werkstatt run agents.generate --site forge-site` | After shared AGENTS.md patterns change (RFC-0079) |
